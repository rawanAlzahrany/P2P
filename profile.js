...
// ========== EMAIL VALIDATION ==========
function isValidUJEmail(email) {
  // Matches 7 digits followed by @uj.edu.sa
  // ^ start, \d{7} = exactly 7 digits, @uj.edu.sa, $ = end
  const regex = /^\d{7}@uj\.edu\.sa$/;
...
function createProfilePanel() {
  ...
  if (!isValidUJEmail(email)) {
    alert('Email must be exactly 7 digits followed by @uj.edu.sa (e.g., 1234567@uj.edu.sa)');
    return;
  }
